...
const ABI = [
  ...
  {
    inputs: [
      { internalType: 'address', name: '_buyer_address', type: 'address' }
    ],
    name: 'getEscrowInfo',
    outputs: [
  ...
  },
  {
    inputs: [
      { internalType: 'address', name: '_buyer_address', type: 'address' },
      { internalType: 'address', name: '_seller_address', type: 'address' }
    ],
    name: 'setPrintShipped',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function'
  },
...
const ADDRESS = '0x09642bbf6BC5eAfa739125723398f8eDb5C60e8C'
...
class EscrowService {
  ...
  purchaseOrder = async function(buyerAddress, sellerAddress, price) {
    ...
    contract.methods
      .createNewEscrow(buyerAddress, sellerAddress, price)
      .send({
        from: buyerAddress,
        value: price,
        gas: 3000000
  ...
  stakeSeller = async function(buyerAddress, sellerAddress, stake) {
    ...
    const contract = await new web3.eth.Contract(ABI, ADDRESS)
    contract.methods
      .stakeSeller(buyerAddress, sellerAddress, stake)
      .send({
        from: sellerAddress,
        value: stake,
        gas: 3000000
  ...
  markShipped = async function(buyerAddress, sellerAddress) {
    ...
    const contract = await new web3.eth.Contract(ABI, ADDRESS)
    contract.methods
      .setPrintShipped(buyerAddress, sellerAddress)
      .send({
        from: sellerAddress,
        gas: 3000000
      })
  ...
  closeDispute = async function(buyerAddress, sellerAddress) {
    ...
    const contract = await new web3.eth.Contract(ABI, ADDRESS)
    contract.methods
      .closeDispute(buyerAddress, sellerAddress)
      .send({
        from: account,
        gas: 3000000
      })
